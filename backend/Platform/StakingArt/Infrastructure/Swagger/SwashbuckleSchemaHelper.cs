...
public class SwashbuckleSchemaHelper
{
    ...
    public string GetSchemaId(Type modelType)
    {
        ...
        {
            x = GetPrefix(modelType.GenericTypeArguments.First()!.Namespace);
        }
        else
        {
            x = GetPrefix(modelType.Namespace);
        }

        return $"{x}{id}";
    }

    string GetPrefix(string strNamespace)
    {
        var x = string.Empty;

        if (strNamespace.Contains("admin", StringComparison.OrdinalIgnoreCase))
            x = "Admin_";

        if (strNamespace.Contains("app", StringComparison.OrdinalIgnoreCase))
            x = "App_";
